...
export default function ModalNewOrder({
  ...
  async function findProduct() {
    ...
    if (response !== undefined) {
      if (product.length === 0) {
        ...
        product.every((prod) => prod?.itemCode !== response?.data?.itemCode)
      ) {
        setProduct([response?.data, ...checkedProduct]);
      } else {
        toast({
  ...
  function handleProductCheck(id: string, checked: boolean) {
    ...
    if (checked) {
      ...
      const newChecked = checkedProduct.filter((prod) => prod?.itemCode !== id);
  ...
  function newItens() {
    const hasremaingItens = itens?.orders.some((item: any) => {
      const hasCheckYet = checkedProduct.some(
        (check) =>
          check?.itemCode === item?.itemCode && item?.remaining === true
      );
        ...
        <ModalContent h={wideVersion ? "" : "500px"}>
            ...
            >
              <Flex gap={"1rem"}>
                <InputWithLabel
                  ...
                  value={itemId}
                  setValue={setItemId}
                  text={"ESCREVA O NÚMERO DO ITEM PARA BUSCAR"}
                  width={"full"}
                  textColor={"black"}
                  fontSize={"12px"}
                />
                      ...
                      >
                        <Flex w={"20%"} justify="space-around">
                          <Checkbox
                            ...
                            value={prod?.itemCode}
                            isChecked={checkIsChecked(prod?.itemCode)}
                            onChange={(event) =>
                              handleProductCheck(
                                prod?.itemCode,
                          ...
                          />
                          <Text alignSelf={"center"}>{prod?.itemCode}</Text>
                          <Text></Text>
                        </Flex>
                        <Flex w={"40%"} justify="center">
                        ...
                        <Flex w={"20%"} justify="center">
                          <Text alignSelf={"center"}>
                            {checkedProduct.some(
                              (checkProd) =>
                                checkProd?.itemCode === prod?.itemCode
                            ) ? (
                              prod?.qty === 0 ? (
                                <ChangeQty
                                  index={index}
                                  arrayItens={product}
                                  setArrayItens={setProduct}
                                  id={prod?.itemCode}
          ...
          <ModalFooter>
            <Flex justify={"space-between"} w={"100%"}>
              ...
              <Button
                isDisabled={checkedProduct.length > 0 ? false : true}
                // w="full"
                bgColor={"#005F27"}
                color="white"
                textStyle={"MontserratBold"}
