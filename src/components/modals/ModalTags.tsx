  ...
  function checkeds() {
    ...
    for (let i = 0; i < allCheckedsNumber; i += 1) {
      if (allCheckedsNumber !== 0) {
        teste.push(
          <Flex p="1rem" direction="column" align={"center"}>
            <Flex direction="column">
              <Flex direction="row" align="end" fontSize="18px">
                <Img src="/Image/rs-black.svg" w="140px" h="40px" mr="4rem" />
                <Text fontWeight="bold">PED: {orderNumber}</Text>
              </Flex>
              <Flex mt="1rem" direction="column" fontSize="18px">
                <Text fontWeight="bold">
                  {itens?.orders && itens?.orders[0].shippingCompany}
                </Text>
                <Text fontWeight="bold">
                  {itens?.orders && itens?.orders[0].city.replace(" ", "")}
                </Text>
                <Text fontWeight="bold">
