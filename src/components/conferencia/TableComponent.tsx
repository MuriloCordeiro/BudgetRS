        ...
        <Tbody>
          {arrayItens?.orders.map((prod, index) => (
            <Tr key={index}>
              <Td w="3%">
                <BsTrash
                  visibility={prod?.remaining ? "visible" : "hidden"}
                  cursor={"pointer"}
                  onClick={() =>
              ...
              <Td w="10%">
                <Tooltip
                  label={prod?.remaining ? "Este é um item pendente" : ""}
                  borderRadius={"15px"}
                >
                  <Text
                    cursor={prod?.remaining ? "help" : "false"}
                    textAlign={"center"}
                    bgColor={prod?.remaining ? "#F9B000" : "none"}
                    padding={"5px"}
                    borderRadius={"15px"}
                  >
                    {prod?.itemCode}
              ...
              <Td w="11%">{prod?.barcode ? prod?.barcode : "-"}</Td>
              <Td w="12%">{prod?.qty}</Td>
              <Td w="12%">
